...
<main class="container my-3 my-sm-4">
  <div class="d-flex justify-content-between align-items-end">
    <h1 class="text-warning d-flex align-items-center IBMPlexMonoFont m-0"><ion-icon name="checkbox" class="me-2 page-identificator-icon"></ion-icon>CLASSIFICAR ITEM COMO DEVOLVIDO</h1>
  ...
  <span class="text-secondary mb-4 d-block text-center text-sm-start">Nesta página, você pode classificar itens encontrados como devolvidos.</span>
  <?php
  if (isset($_GET['alert'])) {
    echo '<div class="alert alert-danger d-flex align-items-center justify-content-between fw-semibold alert-max-width mx-auto" role="alert">
          ...
          <a href="moveFoundItem.php" class="btn-close"></a>
        </div>';
  }
  ?>
  <div class="w-100 mx-auto crud-form-container mt-md-5">
    <form action="crudValidation\moveFoundItemValidation.php" method="post" class="needs-validation" novalidate>
      <?php
      echo '<input name="id" type="hidden" value="' . $_POST['id'] . '">
    <div class="form-floating mb-3">
      <input name="name" id="name" class="form-control" placeholder="Nome do item" value="' . $_POST['name'] . '" required>
      <label for="name">Nome do item</label>
    </div>
    <div class="form-floating mb-3">
      <textarea name="description" class="form-control" placeholder="Descrição do item" id="description" style="height: 100px" required>' . $_POST['description'] . '</textarea>
      <label for="description">Descrição do item</label>
    </div>
    <div class="mb-3 row row-gap-3">
      <div class="col-md">
        <div class="form-floating">
          <input type="datetime-local" name="dateOfReturn" id="dateOfReturn" class="form-control" placeholder="Data de devolução" required>
    ...
    <div class="row gap-1">
      <input type="submit" value="Mover item" class="btn btn-success col-md">
