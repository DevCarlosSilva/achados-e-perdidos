...
<main class="container my-3 my-sm-4">
  <div class="d-flex justify-content-between align-items-end">
    <h1 class="text-warning d-flex align-items-center IBMPlexMonoFont m-0"><ion-icon name="brush-outline" class="me-2 page-identificator-icon"></ion-icon>EDITAR ITEM DEVOLVIDO</h1>
  ...
  <span class="text-secondary mb-4 d-block text-center text-sm-start">Nesta página, você pode editar um item devolvido.</span>
  <?php
  if (isset($_GET['alert'])) {
    echo '<div class="alert alert-danger d-flex align-items-center justify-content-between fw-semibold alert-max-width mx-auto" role="alert">
          ...
          <a href="editReturnedItem.php?id=' . $_GET['id'] . '&name=' . $_GET['name'] . '&description=' . $_GET['description'] . '&receiverName=' . $_GET['receiverName'] . '&dateOfReturn=' . $_GET['dateOfReturn'] . '&id_category=' . $_GET['id_category'] . '" class="btn-close"></a>
        </div>';
  }
  ?>
  <div class="w-100 mx-auto crud-form-container mt-md-5">
    <form action="crudValidation\editReturnedItemValidation.php" method="post" class="needs-validation" novalidate>
      <?php
      ...
      <div class="mb-3 row row-gap-3">
        ...
        <div class="col-md">
          <div class="form-floating">
            <input name="receiverName" id="receiverName" class="form-control" placeholder="Nome do receptor" value="' . $_GET['receiverName'] . '" required>
